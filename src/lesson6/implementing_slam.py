...
def slam(data, N, num_landmarks, motion_noise, measurement_noise):
    ...
    #create constraint matrix and vector
    omega = matrix()
    omega.zero(dim, dim)
    omega.value[0][0] = 1.0
    omega.value[1][1] = 1.0
    ...
    for i in range(len(data)):
        #n is the index of the robot pose in the matrix or vector
        n = 2 * i
        ...
        for j in range(len(measurement)):
            #m is the index of the landmark coordinate in the matrix or vector
            m = 2 * (N + measurement[j][0])
            
            #update the information matrix/vector based on the measurement
            for k in range(2):
                omega.value[n + k][n + k] += 1.0 / measurement_noise
                omega.value[m + k][m + k] += 1.0 / measurement_noise
                omega.value[n + k][m + k] += -1.0 / measurement_noise
                omega.value[m + k][n + k] += -1.0 / measurement_noise
                
                xi.value[n + k][0] += -measurement[j][1 + k] / measurement_noise
                xi.value[m + k][0] += measurement[j][1 + k] / measurement_noise
                
        #update the information matrix/vector base on robot motion
        for b in range(4):
