...
class robot:
    ...
    def move(self, motion, threshold=0.001): 
        ...
        #set noise
        turn2 = random.gauss(turn, self.steering_noise) 
        distance2 = random.gauss(distance, self.distance_noise)          
        
        beta = (distance / self.length) * tan(turn2) 

        if abs(beta) < threshold:
            x = self.x + distance2 * cos(self.orientation)
            y = self.y + distance2 * sin(self.orientation)
        else:
            R = distance2 / beta
